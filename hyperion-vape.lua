...
CameraGroup:AddToggle("CameraEnabledToggle", {
	...
	do
		-- The Accuracy Range widget is a Frost-GUI-specific custom control
		-- (it parents frames into the groupbox button's hierarchy). On the
		-- Vape UI the mock button cannot accept Instance parents, so the
		-- widget body is executed in a protected call; if the GUI-specific
		-- layout fails, the range values still work through the
		-- LowSpeedRandomizedToggle callback and the config.
		pcall(function()
